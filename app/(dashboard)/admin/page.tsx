...
// Trang quan tri he thong (Admin Dashboard)
// Noi quan ly nguoi dung, phan quyen va theo doi he thong.
      ...
      <div className="flex flex-col items-center justify-center text-center max-w-md mx-auto py-20 px-4 select-none">
        ...
        <div className="mt-6 flex flex-col gap-2.5 w-full">
          <button
            ...
            className="w-full py-2.5 bg-primary text-primary-foreground font-bold text-sm rounded-lg hover:bg-primary/95 transition-all shadow-premium-sm flex items-center justify-center gap-1.5 cursor-pointer"
          >
    ...
    <div className="space-y-6">
        ...
        {[
          ...
          return (
            <button
              ...
              className={cn(
                "flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-bold transition-all cursor-pointer border",
                isActive
      ...
      {loading ? (
        ...
        <>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 select-none">
                <div className="bg-card border border-border/80 rounded-xl p-5 shadow-premium-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-muted-foreground uppercase">Users Hoạt động</span>
                    <Users className="h-4.5 w-4.5 text-primary" />
                  </div>
                  <h3 className="text-xl font-bold tracking-tight mt-3">{stats.activeUsers} nhân viên</h3>
                  <p className="text-xs text-muted-foreground mt-1">Hoạt động trong 15 phút qua</p>
                </div>
                <div className="bg-card border border-border/80 rounded-xl p-5 shadow-premium-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-muted-foreground uppercase">Chi phí Compute (VND)</span>
                    <DollarSign className="h-4.5 w-4.5 text-emerald-500" />
                  </div>
                  <h3 className="text-xl font-bold tracking-tight mt-3">{formatCurrency(stats.totalCost)}</h3>
                  <p className="text-xs text-emerald-500 font-bold mt-1 inline-flex items-center gap-0.5">
                    <span>+12.4%</span>
                ...
                <div className="bg-card border border-border/80 rounded-xl p-5 shadow-premium-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-muted-foreground uppercase">Số yêu cầu API</span>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Độ chính xác mô hình: 99.4%</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-card border border-border/80 rounded-xl p-4 shadow-premium-sm space-y-4">
                  <h3 className="text-sm font-bold text-foreground flex items-center justify-between select-none">
                    <span>Lịch sử Chi phí API theo ngày</span>
                    <span className="text-xs text-muted-foreground font-normal">Đơn vị: VNĐ</span>
                  </h3>
                ...
                <div className="bg-card border border-border/80 rounded-xl p-4 shadow-premium-sm space-y-4">
                  <h3 className="text-sm font-bold text-foreground flex items-center justify-between select-none">
                    ...
                    <span className="text-xs text-muted-foreground font-normal">Đơn vị: Lượt gọi</span>
                  </h3>
                  <div className="h-64 w-full flex items-center justify-center text-sm">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 select-none">
                <div className="flex items-center gap-4">
                  <h3 className="text-sm font-bold text-foreground">Quản lý định biên & Tài khoản</h3>
                  <button 
                    onClick={() => alert("Chức năng Thêm người dùng mới")}
                    className="flex items-center gap-1.5 px-2.5 py-1.5 bg-primary text-primary-foreground text-xs font-bold rounded-lg hover:bg-primary/95 transition-all shadow-premium-sm cursor-pointer">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="px-2 py-1.5 text-sm bg-card border border-border rounded-lg outline-none focus:border-primary text-foreground cursor-pointer"
                  >
                  ...
                  <div className="relative w-full max-w-xs">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-1.5 text-sm bg-card border border-border rounded-lg outline-none focus:border-primary text-foreground"
                    />
                  </div>
                </div>
              </div>
              ...
              {/* Pagination Controls */}
              {totalPages > 1 && (
                <div className="flex items-center justify-between pt-2 text-sm text-muted-foreground select-none">
          ...
          {activeTab === "audit" && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-foreground select-none">Nhật ký truy cập bảo mật</h3>
              
              <div className="bg-card border border-border rounded-xl divide-y divide-border/60 overflow-hidden shadow-premium-sm">
                {auditLogs.map((log) => (
                  <div key={log.id} className="p-3.5 flex items-center justify-between hover:bg-secondary/25 transition-colors text-sm">
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-card border border-border shadow-premium-lg rounded-xl p-6 outline-none z-50">
            ...
            <form onSubmit={handleSaveUser} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground">Phòng ban</label>
                <input
                  type="text"
                  value={userForm.dept}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-muted-foreground">Quyền hạn (Role)</label>
                  <select
                    value={userForm.role}
              ...
              <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-border/60">
                <Dialog.Close asChild>
                  <button type="button" className="px-3 py-1.5 rounded-lg border border-border hover:bg-secondary text-sm font-bold cursor-pointer">Hủy bỏ</button>
                </Dialog.Close>
                <button type="submit" className="px-3 py-1.5 rounded-lg bg-primary hover:bg-primary/95 text-primary-foreground text-sm font-bold shadow-premium-sm cursor-pointer">Lưu thay đổi</button>
              </div>
            </form>
          </Dialog.Content>
        </Dialog.Portal>
